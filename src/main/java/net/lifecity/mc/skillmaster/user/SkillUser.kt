...
class SkillUser(
     ...
     * @param vector ノックバック
     */
    fun damageAddVector(attackUser: SkillUser, damage: Double = 0.0, vector: Vector = Vector(0.0, 0.0, 0.0), noDefense: Boolean, atkLoc: Location) {
        if (noDefense || !canDefense(attackUser, damage, vector, atkLoc)) {
            // ダメージとノックバックを与える
     ...
     * @param vector ノックバック
     */
    fun damageChangeVector(attackUser: SkillUser, damage: Double = 0.0, vector: Vector, noDefense: Boolean = false, atkLoc: Location) {
        if (noDefense || !canDefense(attackUser, damage, vector, atkLoc)) {
            // ダメージとノックバックを与える
            player.damage(damage)
     ...
     * @param vector ノックバック
     */
    private fun canDefense(attackUser: SkillUser, damage: Double, vector: Vector, atkLoc: Location): Boolean {
        // 防御スキル取得
        val activatedSkill: CompositeSkill? = getActivatedSkill()
        ...
        if (activatedSkill != null) {
            if (activatedSkill is Defense) {
                activatedSkill.defense(attackUser, damage, vector, atkLoc)
                return true
            }
        }
        return false
